...
- 클라이언트의 요청을 처리하는 커넥션 핸들러, SQL 파서 및 전처리기, 옵티마이저로 구성되어있다.
- 요청된 SQL 문장을 분석하거나 최적화하는 등 DBMS의 두뇌에 해당하는 기능을 수행한다.
...
```markdown
- 테이블의 데이터 검색 속도를 향상시키기 위해 사용되는 데이터 구조
- 칼럼의 값과 헤당 레코드가 저장된 주소를 키와 값의 쌍으로 만들어 저장하며,
  칼럼의 값은 주어진 순서로 미리 정렬해서 보관하여 최대한 빠르게 찾아갈 수 있도록 한다
- 인덱스를 관리하기 위해 DB의 약 10%에 해당하는 저장공간이 필요하며 관리를 위한 추가 작업이 필요함
```
...
```markdown
- 랜덤 I/O 작업을 여러번 호출할 시 디스크 헤더를 이동시킨 후 데이터를 읽는데, 이 과정에서 많은 시간이 소요
- 디스크 원판을 가지지 않는 SSD에서도 이는 마찬가지, 여전히 순차 I/O보다 Throughput이 떨어진다
- 인덱스를 도입하면 디스크 헤더가 모든 데이터를 순차적으로 탐색하는 Full Table Scan 대신
  인덱스 트리를 따라 필요한 데이터만 접근하므로 디스크 헤더 이동을 최소화
```
...
```markdown
- 인덱스는 데이터 삽입, 수정, 삭제 시마다 인덱스 구조를 업데이트해야 하기 때문에 추가적인 디스크 I/O와 연산 비용이 발생
- 수정이 잦은 테이블에 인덱스를 사용하면 이러한 갱신 작업이 누적되어 쓰기 성능이 크게 저하됨
- 따라서 읽기(Read) 작업이 많고 쓰기(Write) 작업이 적은 테이블에 인덱스를 사용하는 것이 효율적
```
...
```markdown
인덱스가 존재하면 ORDER BY/GROUP BY 연산에서 정렬과 중복 처리 비용이 감소하여 성능이 크게 향상된다.

# 인덱스가 없는 경우

- DBMS는 테이블의 데이터를 메모리나 임시 테이블에 모두 로드한 뒤 정렬
- 이 과정에서 Full Table Scan이 발생하며, 정렬 연산에 많은 리소스와 시간이 소요

# 인덱스가 있는 경우

- 인덱스는 데이터를 특정 열 기준으로 정렬된 상태로 저장하므로, ORDER BY 또는 GROUP BY에서 추가적인 정렬 작업을 생략하거나 최소화
- 예를 들어, B-Tree 인덱스는 데이터를 오름차순 또는 내림차순으로 정렬된 상태로 유지하므로 ORDER BY에 적합
- GROUP BY 연산에서도 중복 값을 효율적으로 탐지하여 결과를 빠르게 집계할 수 있음
```
...
```markdown
기본키는 인덱스와 같지 않다.
기본키는 테이블의 각 레코드를 고유하게 식별하는 제약 조건이며, 인덱스는 데이터 검색을 빠르게 하기 위한 데이터 구조일 뿐이다.

# 차이점

- 기본키는 중복되지 않는 고유 값을 저장하며 NULL을 허용하지 않지만, 인덱스는 제약조건을 강제하지 않는다.
- 테이블에는 하나의 기본키만 있을 수 있지만, 인덱스는 여러개를 생성할 수 있다.
```
